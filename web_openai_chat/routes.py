...
def index():
    ...
    # Map each country to a query for an iconic landmark
    country_query_map = {
    ...
    for destination in popular_destinations:
        query = country_query_map.get(destination.country, f"{destination.country} iconic landmark")
        try:
            ...
            if results:
                destination.image_url = results[0].get("urls", {}).get("regular", "https://via.placeholder.com/600x300?text=No+Image+Available")
            else:
...
def search_flights():
    ...
    current_date = datetime(2025, 3, 25)  # System date as per your setup
...
def my_bookings():
    ...
    return render_template('manage_bookings.html', bookings=bookings, now=current_time)
...
def admin_flights():
    ...
    flights = query.order_by(Flight.departure_time).paginate(page=page, per_page=20)
...
def admin_users():
    ...
    # Pagination
    page = request.args.get('page', 1, type=int)
    users = query.order_by(User.date_joined.desc()).paginate(page=page, per_page=20)
...
def admin_bookings():
    ...
    # Pagination
    page = request.args.get('page', 1, type=int)
    bookings = query.order_by(Booking.booking_date.desc()).paginate(page=page, per_page=20)
...
def send_email_in_background(app, user_data, email_type):
    """
    Send email in a separate thread with application context.
    app: Flask app instance
    user_data: Dictionary containing user details
    email_type: Type of email to send ("login", "welcome", "logout")
    """
    def send_email():
        with app.app_context():  # Set up application context in the thread
            try:
                openrouter_api_key = os.getenv("OPENROUTER_API_KEY")
                if not openrouter_api_key:
                ...
                # API call to generate email content
                response = requests.post(
                    "https://openrouter.ai/api/v1/chat/completions",
                    headers=headers,
                    json={
                ...
                # Log the error instead of flashing
